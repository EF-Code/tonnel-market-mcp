...
function setup() {
  ...
  return { database, events, projections: new ProjectionRepository(), query: new QueryRepository(database.db) };
}
...
test('projects every known event and counts canonical sales once', () => {
  ...
  assert.equal((database.db.prepare('SELECT COUNT(*) AS count FROM raw_events').get() as { count: number }).count, 28);
  assert.equal((database.db.prepare('SELECT COUNT(*) AS count FROM sale_facts').get() as { count: number }).count, 1);
  assert.equal((database.db.prepare('SELECT COUNT(*) AS count FROM auction_bids').get() as { count: number }).count, 1);
  assert.equal(events.getHealthState().unknownEventCount, 1);
  assert.equal(query.sales({}).length, 1);
  assert.equal(query.listAuctions({ auctionId: 'auction-1', limit: 10 })[0]?.bidCount, 1);
  const search = query.search({ giftId: 123, asset: 'TON', limit: 100, sort: 'occurred_asc' });
...
test('rolls back projections and does not advance the checkpoint on failure', () => {
  ...
  const event = allKnownFixtures.find((item) => item.type === 'sale.completed')!;
  assert.throws(() =>
    events.processEvent(
      event,
      {
  ...
  assert.equal((database.db.prepare("SELECT processing_status FROM raw_events WHERE event_id = ?").get(event.eventId) as { processing_status: string }).processing_status, 'failed');
