...
import { allKnownFixtures, connectedFixture, fixtureEvent, fixtureGift, unknownFixture } from '../fixtures/events.js';
...
test('accepts additive fields while preserving them in the event data', () => {
  const event = parseMarketplaceEvent({
    ...
    data: { ...fixtureEvent('listing.created').data, future_field: { safe: true } },
  });
  assert.deepEqual((event.data as Record<string, unknown>).future_field, { safe: true });
});
...
test('preserves unknown event types as raw-compatible events', () => {
  ...
  assert.equal((event.data as Record<string, unknown>).gift_id, fixtureGift.gift_id);
});
...
test('rejects incompatible envelope versions safely', () => {
  assert.throws(
    ...
    (error: unknown) => isAppError(error) && error.code === 'UNSUPPORTED_EVENT_VERSION',
  );
});
