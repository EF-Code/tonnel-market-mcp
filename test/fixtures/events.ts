...
const fixtureData: Record<KnownEventType, Record<string, unknown>> = {
  ...
  "listing.promoted": {
    gift: fixtureGift,
    target: "LISTING",
    promotion_started_at: "2026-08-15T00:00:00.000Z",
  },
  "listing.promotion_ended": {
    target: "LISTING",
    count: 2,
    expired_before: "2026-08-15T00:00:00.000Z",
  },
  "sale.completed": {
    gift: fixtureGift,
    price: 11,
...
export function fixtureEvent(
  ...
  const parsed = parseMarketplaceEvent({
    ...
    type,
    occurredAt: new Date(
      Date.parse("2026-08-15T00:00:00.000Z") + suffix * 1_000,
    ).toISOString(),
...
export const allKnownFixtures = EVENT_TYPES.map((type, index) =>
  fixtureEvent(type, index),
);
